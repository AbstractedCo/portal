...
import { selectedDaoIdAtom } from '../../../features/daos/store'
...
export const Route = createFileRoute('/daos/_layout/settings')({
  component: DaoParameters,
  beforeLoad: () => ({ title: 'DAO Parameters' }),
})

function DaoParameters() {
  const { showNotification } = useNotification()
  const [isEditing, setIsEditing] = useState(false)
  const [_isProcessing, setIsProcessing] = useState(false)
  const selectedDaoId: number | undefined = useAtomValue(selectedDaoIdAtom)
  if (typeof selectedDaoId !== 'number') {
    return null
  }
